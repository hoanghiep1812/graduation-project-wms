...
@section('title', 'Sửa Sản Phẩm: ' . $product->sku)
        ...
        <div class="col-lg-8 mx-auto">
            ...
            <div class="card card-flush">
                <div class="card-header pt-5">
                    <h3 class="card-title text-gray-800 fw-bold">Cập Nhật Sản Phẩm</h3>
                </div>
                ...
                <div class="card-body pt-5">
                    <form action="{{ route('admin.products.update', $product->id) }}" method="POST">
                        ...
                        <div class="row mb-8">
                            <div class="col-md-6">
                                <label class="required fs-6 fw-semibold mb-2">Đơn Vị Tính</label>
                                <select name="unit" class="form-select form-select-solid" data-control="select2"
                                    data-hide-search="true" required>
                                    <option value="Cái" {{ old('unit', $product->unit) == 'Cái' ? 'selected' : '' }}>Cái
                                    </option>
                                    <option value="Chiếc" {{ old('unit', $product->unit) == 'Chiếc' ? 'selected' : '' }}>Chiếc
                                    </option>
                                    <option value="Bộ" {{ old('unit', $product->unit) == 'Bộ' ? 'selected' : '' }}>Bộ</option>
                                    <option value="Hộp" {{ old('unit', $product->unit) == 'Hộp' ? 'selected' : '' }}>Hộp
                                    </option>
                                    <option value="Thùng" {{ old('unit', $product->unit) == 'Thùng' ? 'selected' : '' }}>Thùng
                                    </option>
                                </select>
                            </div>

                            <div class="col-md-6">
                                <label class="fs-6 fw-semibold mb-2">Tồn Kho Tối Thiểu</label>
                                ...
                                <div class="d-flex flex-stack mb-4">
                                    ...
                                    <label class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" name="has_expiry"
                                            id="hasExpiryCheckbox" value="1" {{ old('has_expiry', $product->has_expiry) ? 'checked' : '' }} />
                                    </label>
                                </div>
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-md-8">
                                            ...
                                            <input type="number" class="form-control form-control-solid"
                                                name="expiry_duration" min="1"
                                                value="{{ old('expiry_duration', $product->expiry_duration) }}" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="text-end mt-10">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="ki-duotone ki-save-2 fs-2"><span class="path1"></span><span
                                        class="path2"></span></i> Cập Nhật
                            </button>
                        </div>
                    </form>
                </div>
